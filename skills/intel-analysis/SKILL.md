...
| daily-loop | 每日盤點驅動契約 | 紀律 19-23 帳本側 | 今天該盤誰?誰到期?誰該休眠?(資訊獲取由使用者自理) | [`references/daily-loop.md`](references/daily-loop.md) |
...
python3 scripts/intel_daily.py brief --repo .          # 每日盤點簡報(帳本側;資訊獲取由使用者自理,見 daily-loop)
python3 scripts/intel_daily.py log --repo . --chain P-… --outcome A|B|C|D   # 處置記錄(紀律 19d)
```
