...
| Skill | 說明 | 入口 |
|-------|------|------|
| **ai-sdlc**(單人版,基底) | AI 開發治理閉環:需求分析 → 結構設計 → 修改治理 → 驗收;含抗漂移與選用 CI/CD;適合一人/單一 agent,也是團隊版的基底 | [`skills/ai-sdlc/SKILL.md`](skills/ai-sdlc/SKILL.md) |
| **ai-sdlc-team**(團隊版) | 在 ai-sdlc 之上加多代理/多 session 協作交接、獨立驗收、角色與讀寫權限;適合團隊(可為 AI agent 團隊) | [`skills/ai-sdlc-team/SKILL.md`](skills/ai-sdlc-team/SKILL.md) |

`ai-sdlc`(單人版/基底)= 四階段 + 兩份跨階段指引:
...
| ✦ CI/CD [`ci-cd`](skills/ai-sdlc/references/ci-cd.md)(**選用**) | 依單人/團隊需求把門檻自動化 |

> 抗 session 壓縮:基底含「Session 啟動檢查 + 不倚賴記憶以文件為準」。跨專案:Guideline/CHG/ACC/結構文件標頭都有「專案」欄。

`ai-sdlc-team`(團隊版)= 協作層,**搭配基底使用**。「團隊」不限於人,**可以是 AI agent 團隊**。兩份 references(抗漂移與 CI/CD 用基底的):
...
└── skills/                        # Skill 原始檔
    ...
    │   │   ├── doc-integrity.md / .zh-tw.md         # 文檔抗漂移與驗證(跨階段)
    │   │   └── ci-cd.md / .zh-tw.md                 # CI/CD 整合(選用)
    │   └── evals/evals.json       # 測試案例定義
    └── ai-sdlc-team/              # 團隊版(協作層,搭配 ai-sdlc;團隊可為 AI agent)
        ├── SKILL.md / SKILL.zh-tw.md           # orchestrator(英文 / 繁中)
        └── references/
            ├── cross-agent.md / .zh-tw.md            # 跨 agent 協作與交接(角色+讀寫權限)
            └── independent-acceptance.md / .zh-tw.md # 跨 agent / 多情境獨立驗收(驗收者唯讀)
```
