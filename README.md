...
兩版都**自我完備、可獨立安裝**,且**共用同一套 references(11 份)與「偵測即載入」機制**——差別只在**預設定位**:`ai-sdlc` 以單人為預設、`ai-sdlc-team` 以多人/多 agent 協作為預設。**偵測機制單人團隊通用**:偵測到多 repo 就載 `cross-repo`、並行/交接就載 `cross-agent`、要派子代理就載 `agent-worklog`/`agent-hierarchy`、提出修改就載 `modification-guide`……(單人也可能遇到多 repo 或並行子代理)。
...
| ✦ 獨立驗收 [`independent-acceptance`](skills/ai-sdlc/references/independent-acceptance.md) | 跨 agent / 多情境獨立驗收(驗收者 ≠ 實作者、且唯讀) |
...
└── skills/                        # Skill 原始檔(兩版 references 相同,差在 orchestrator 定位)
    ├── ai-sdlc/                   # 單人版(自含)
    │   ├── SKILL.md / SKILL.zh-tw.md       # orchestrator(英文 / 繁中;以單人為預設)
    ...
    └── ai-sdlc-team/              # 團隊版(自含;以多人/多 agent 協作為預設)
        ├── SKILL.md / SKILL.zh-tw.md           # orchestrator(英文 / 繁中)
        ├── scripts/cross_repo_check.py
        └── references/                          # 與單人版相同的 11 份
```
